...
function kaprekarSplit(n){
    if (n === 1) return 0;
    const squareNString = `${n * n}`;
    let result = -1;

    for (let i = 0; i < squareNString.length; i++) {
        const firstHalf = squareNString.slice(0, i);
        const secondHalf = squareNString.slice(
            i,
            squareNString[squareNString.length]
        );

        if (Number(firstHalf) + Number(secondHalf) === n) {
            result = i;
            break;
        }
    }
