...
function missingWord(nums, str) {
    ...
    let resultString = '';
    const lowercaseAndRemoveSpaces = [...str.replace(/\s/g, '').toLowerCase()];

    for (let i = 0; i < sortedIndices.length; i++) {
        const num = sortedIndices[i];

        if (!lowercaseAndRemoveSpaces[num]) {
            return 'No mission today';
        } else {
            resultString += lowercaseAndRemoveSpaces[num];
        }
    }
