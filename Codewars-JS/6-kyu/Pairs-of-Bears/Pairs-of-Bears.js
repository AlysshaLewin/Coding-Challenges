...
function bears(x, s) {
    ...
    const validMatingPairs = [];
    let pointerOne = 0;
    let pointerTwo = 1;
  
    while (pointerTwo < s.length) {
      let charOne = s[pointerOne];
      let charTwo = s[pointerTwo];
  
      if (
        (charOne === "8" && charTwo === "B") ||
        (charOne === "B" && charTwo === "8")
      ) {
        validMatingPairs.push(charOne);
        validMatingPairs.push(charTwo);
        countNumberOfPairs++;
        pointerOne += 2;
        pointerTwo += 2;
        continue;
    ...
    return [validMatingPairs.join(""), countNumberOfPairs >= x];
}
